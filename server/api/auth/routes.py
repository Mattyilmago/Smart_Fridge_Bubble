...
from utils.request_auth import get_user_and_body
...
def register_user():
    ...
    Response:
        Success (200): "eyJhbGc..." (user_token JWT)
        Error (400/409/500): {"success": false, "error": {...}}
    """
    try:
        ...
        if not email:
            ...
            return error_response(ErrorCode.MISSING_PARAMETER, "email mancante")
        
        if not password:
            logger.warning("Missing password")
            return error_response(ErrorCode.MISSING_PARAMETER, "password mancante")
        
        # Validazione email minima
        if '@' not in email or len(email) < 5:
            logger.warning(f"Invalid email format: {email}")
            return error_response(ErrorCode.INVALID_REQUEST, "Formato email invalido")
        
        # Validazione password minima
        from config import Config
        if len(password) < Config.MIN_PASSWORD_LENGTH:
            logger.warning("Password too short")
            return error_response(ErrorCode.INVALID_REQUEST, f"Password troppo corta (min {Config.MIN_PASSWORD_LENGTH} caratteri)")
        ...
        if not user_id:
            ...
            return error_response(ErrorCode.INVALID_REQUEST, "Email già registrata"), 409
        
        # Genera user token
        user_token = generate_user_token(user_id)
        
        logger.info(f"User {user_id} registered: {email}")
...
@auth_bp.route('/isAuthorizedUser', methods=['GET'])
def is_authorized_user():
    """
    Valida user_token esistente
    ...
    try:
        ...
        if not payload:
            ...
            logger.warning("User token expired or invalid")
        ...
        if is_token_expiring_soon(payload):
            # Rinnova token
            new_token = generate_user_token(user_id)
            logger.info(f"User token rinnovato per user {user_id} (in scadenza)")
            return new_token, 200
        
        # Token ancora valido, ritorna stesso token
        logger.debug(f"User token valido per user {user_id}")
        return user_token, 200
        
    except Exception as e:
        logger.error(f"Unexpected error in is_authorized_user: {e}")
        return error_response(ErrorCode.INTERNAL_ERROR)


@auth_bp.route('/renewUser', methods=['POST'])
def renew_user():
    """
    ...
    try:
        ...
        # Decodifica token (senza verificare scadenza)
        payload = decode_user_token(user_token)
        ...
        if not db.user_exists(user_id):
            logger.warning(f"User {user_id} not found during token renewal")
            return error_response(ErrorCode.USER_NOT_FOUND)
        
        # Genera nuovo token
        new_token = generate_user_token(user_id)
        
        logger.info(f"User token rinnovato per user {user_id}")
        
        # Ritorna nuovo token
        return new_token, 200
        
    except Exception as e:
        logger.error(f"Unexpected error in renew_user: {e}")
        return error_response(ErrorCode.INTERNAL_ERROR)


# ========================================
# FRIDGE AUTHENTICATION
# ========================================
...
def register_fridge():
    ...
    """
    try:
        # Verifica e estrae user_id e body
...
@auth_bp.route('/isAuthorizedFridge', methods=['GET'])
def is_authorized_fridge():
    """
